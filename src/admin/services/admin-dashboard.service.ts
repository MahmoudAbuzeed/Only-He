...
export class AdminDashboardService {
  ...
  async getSalesAnalytics(period: string = "30d", groupBy: string = "day") {
    try {
      ...
      // Get basic metrics
      const [totalRevenue, totalOrders, previousPeriodRevenue] =
        await Promise.all([
          this.orderRepository
      ...
      return {
        ...
        top_categories: topCategories.map((item) => ({
          ...
          percentage:
            totalCategoryRevenue > 0
              ? (parseFloat(item.revenue) / totalCategoryRevenue) * 100
              : 0,
        })),
        payment_methods: paymentMethods.map((item) => ({
          ...
          percentage:
            totalPaymentCount > 0
              ? (parseInt(item.count) / totalPaymentCount) * 100
              : 0,
        })),
      };
    } catch (error) {
  ...
  async getSystemHealth() {
    try {
      ...
      const recentErrorsCount = await this.orderRepository
        .createQueryBuilder("order")
        .where("order.payment_status = :status", { status: "failed" })
