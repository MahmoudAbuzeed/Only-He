...
export class ProductService {
  ...
  async update(id: number, updateProductDto: UpdateProductDto) {
    try {
      ...
      if (
        ...
        const existingSkuProduct = await this.productRepository.findBySku(
          updateProductDto.sku
        );
  ...
  async reserveStock(productId: number, quantity: number): Promise<boolean> {
    try {
      const result = await this.productRepository.decreaseStock(
        productId,
        quantity
      );
  ...
  async releaseStock(productId: number, quantity: number): Promise<boolean> {
    try {
      const result = await this.productRepository.increaseStock(
        productId,
        quantity
      );
      return result.affected > 0;
